...
def load_data(train_file, test_file,y_name='Species'):
    ...
    test_path = test_file#tf.keras.utils.get_file(TEST_URL.split('/')[-1], TEST_URL)
...
class OneHotPlugin:
 def input(self, inputfile):
    ...
    self.train_file = PyPluMA.prefix()+"/"+parameters["train"]
    self.test_file = PyPluMA.prefix()+"/"+parameters["test"]

 def run(self):
    # Fetch the data
    (train_x, train_y), (test_x, test_y) = load_data(self.train_file, self.test_file)
